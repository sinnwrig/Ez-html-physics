...
// Single-filed and jsmined ( http://code.google.com/p/jsmin-php/ ) by Mr.doo
// Kindly formatted into a semi-readable mess by Kai Angulo
...
b2Settings.b2_linearSlop = 0.005 * 30.0;
...
b2Vec2.prototype = {
    ...
    CrossVF: function (s) { 
        var tX = this.x; this.x = s * this.y; this.y = -s * tX;
    }, 
    ...
    MinV: function (b) { 
        this.x = this.x < b.x ? this.x : b.x; this.y = this.y < b.y ? this.y : b.y;
    },
...
b2Mat22.prototype = {
    ...
    Solve: function (out, bX, bY) { 
        ...
        var a21 = this.col1.y; 
        var a22 = this.col2.y; 
        var det = a11 * a22 - a12 * a21; det = 1.0 / det;
...
b2Math.b2IsValid = function (x) { return isFinite(x); }; 
b2Math.b2Dot = function (a, b) { return a.x * b.x + a.y * b.y; }; 
b2Math.b2CrossVV = function (a, b) { return a.x * b.y - a.y * b.x; }; 
b2Math.b2CrossVF = function (a, s) { var v = new b2Vec2(s * a.y, -s * a.x); return v; }; 
b2Math.b2CrossFV = function (s, a) { var v = new b2Vec2(-s * a.y, s * a.x); return v; }; 
b2Math.b2MulMV = function (A, v) { var u = new b2Vec2(A.col1.x * v.x + A.col2.x * v.y, A.col1.y * v.x + A.col2.y * v.y); return u; }; 
b2Math.b2MulTMV = function (A, v) { var u = new b2Vec2(b2Math.b2Dot(v, A.col1), b2Math.b2Dot(v, A.col2)); return u; }; 
b2Math.AddVV = function (a, b) { var v = new b2Vec2(a.x + b.x, a.y + b.y); return v; }; 
b2Math.SubtractVV = function (a, b) { var v = new b2Vec2(a.x - b.x, a.y - b.y); return v; };
b2Math.MulFV = function (s, a) { var v = new b2Vec2(s * a.x, s * a.y); return v; }; 
b2Math.AddMM = function (A, B) { var C = new b2Mat22(0, b2Math.AddVV(A.col1, B.col1), b2Math.AddVV(A.col2, B.col2)); return C; }; 
b2Math.b2MulMM = function (A, B) { var C = new b2Mat22(0, b2Math.b2MulMV(A, B.col1), b2Math.b2MulMV(A, B.col2)); return C; }; 
b2Math.b2MulTMM = function (A, B) { var c1 = new b2Vec2(b2Math.b2Dot(A.col1, B.col1), b2Math.b2Dot(A.col2, B.col1)); var c2 = new b2Vec2(b2Math.b2Dot(A.col1, B.col2), b2Math.b2Dot(A.col2, B.col2)); var C = new b2Mat22(0, c1, c2); return C; }; 
...
b2Math.b2NextPowerOfTwo = function (x) { x |= (x >> 1) & 0x7FFFFFFF; x |= (x >> 2) & 0x3FFFFFFF; x |= (x >> 4) & 0x0FFFFFFF; x |= (x >> 8) & 0x00FFFFFF; x |= (x >> 16) & 0x0000FFFF; return x + 1; }; 
...
b2PairManager.Hash = function (proxyId1, proxyId2) { 
    ...
    key = key * 2057; key = key ^ ((key >> 16) & 0x0000ffff); 
    return key; 
}
...
var b2BroadPhase = Class.create(); b2BroadPhase.prototype = {
    ...
    MoveProxy: function (proxyId, aabb) {
        ...
        if (proxyId == b2Pair.b2_nullProxy || b2Settings.b2_maxProxies <= proxyId) return;
...
b2Collision.FindMaxSeparation = function (edgeIndex, poly1, poly2, conservative) {
    ...
    if (sPrev > 0.0 && conservative == false) return sPrev;
...
b2Shape.PolyCentroid = function (vs, count, out) {
    ...
    for (var i = 0; i < count; ++i) { 
        ...
        var triangleArea = 0.5 * D; 
        area += triangleArea; cX += triangleArea * inv3 * (p1X + p2X + p3X); 
...
Object.extend(b2CircleShape.prototype, b2Shape.prototype); Object.extend(b2CircleShape.prototype, {
    ...
    initialize: function (def, body, localCenter) {
        ...
        this.m_maxRadius = Math.sqrt(rX * rX + rY * rY) + 
        this.m_radius; var aabb = new b2AABB(); 
    ...
    Synchronize: function (position1, R1, position2, R2) {
        ...
        var upperX = Math.max(p1X, this.m_position.x); var upperY = Math.max(p1Y, this.m_position.y); 
        var aabb = new b2AABB(); 
    ...
    Support: function (dX, dY, out) { 
        var len = Math.sqrt(dX * dX + dY * dY); dX /= len; 
        dY /= len; 
